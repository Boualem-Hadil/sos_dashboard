...
export default function SettingsPage() {
  ...
  const Section = ({ icon: Icon, title, children }: { icon: React.ElementType; title: string; children: React.ReactNode }) => (
    <div className="rounded-xl overflow-hidden border" style={{ background: 'var(--sos-bg-surface)', borderColor: 'var(--sos-border)', boxShadow: 'var(--sos-shadow)' }}>
      <div className="flex items-center gap-3 px-6 py-4" style={{ borderBottom: '1px solid var(--sos-border)' }}>
        ...
        <h2 className="text-base font-bold" style={{ color: 'var(--sos-text-primary)' }}>{title}</h2>
      </div>
  ...
  const Field = ({ label, value, onChange, disabled }: { label: string; value: string; onChange?: (v: string) => void; disabled?: boolean }) => (
    <div>
      <label className="block text-xs font-semibold mb-1.5" style={{ color: 'var(--sos-text-secondary)' }}>{label}</label>
      <input value={value} onChange={e => onChange?.(e.target.value)} disabled={disabled}
        className="w-full px-4 py-2.5 rounded-xl text-sm outline-none transition-all"
        style={{ 
          background: disabled ? 'var(--sos-bg-hover)' : 'var(--sos-bg-surface-2)', 
          border: '1px solid var(--sos-border)', 
          color: disabled ? 'var(--sos-text-muted)' : 'var(--sos-text-primary)' 
        }} />
    </div>
  );

  const Toggle = ({ label, desc, checked, onChange }: { label: string; desc: string; checked: boolean; onChange: (v: boolean) => void }) => (
    <div className="flex items-center justify-between py-3" style={{ borderBottom: '1px solid var(--sos-border-subtle)' }}>
      <div>
        <div className="text-sm font-medium" style={{ color: 'var(--sos-text-primary)' }}>{label}</div>
        <div className="text-xs mt-0.5" style={{ color: 'var(--sos-text-secondary)' }}>{desc}</div>
      </div>
      <button onClick={() => onChange(!checked)} className="w-12 h-6 rounded-full relative transition-all"
        style={{ background: checked ? '#E53935' : 'var(--sos-border)' }}>
        ...
        <Section icon={Shield} title="Officiers de Sécurité">
          <div className="flex flex-col gap-3">
            {SAFETY_OFFICERS.map(o => (
              <div key={o.id} className="flex items-center justify-between p-4 rounded-xl border" style={{ background: 'var(--sos-bg-surface-2)', borderColor: 'var(--sos-border)' }}>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-sm font-semibold" style={{ color: 'var(--sos-text-primary)' }}>{o.name}</div>
                    <div className="text-xs" style={{ color: 'var(--sos-text-secondary)' }}>{o.role} — {o.phone}</div>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-xs px-2 py-1 rounded" style={{ background: o.receivesAlerts ? 'rgba(76,175,80,0.15)' : 'var(--sos-bg-hover)', color: o.receivesAlerts ? '#4CAF50' : 'var(--sos-text-muted)', border: `1px solid ${o.receivesAlerts ? '#4CAF5040' : 'var(--sos-border)'}` }}>
